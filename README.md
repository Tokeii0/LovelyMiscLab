...
- 🔗 **流程编排器** —— 整图执行，带内容寻址的增量缓存（打开「实时」开关即改哪算哪）
...
- ⚡ **增量执行** —— 内容寻址缓存，改动后只重算受影响的节点；打开「实时」开关即自动重算。
...
- **运行 / 实时** —— 「运行」（`Ctrl / Cmd + Enter`）整图跑一次；打开「实时」开关后改参即自动重算（耗时节点及其下游跳过，需手动运行）。「停止」只取消、保留已出的结果，「清除结果」才清空
- **整理节点**（画布右键）—— 一键把图按数据流从左到右排布到当前视口
- **运行历史与运行到节点** —— 回看每次执行、只跑到某个节点为止
- **撤销 / 重做** —— 一次拖动、一段输入各算一步（AI Agent 的整次搭建折叠为一步）
- **不丢工作** —— 新建 / 打开 / 模板 / 关窗前提示未保存；另存为（`Ctrl / Cmd + Shift + S`）；自动保存草稿，启动时可恢复
- **最近项目** —— 快速重开
- **帮助面板与边上数据预览** —— 连线上直接看流过的值
- **AI 解释 / 修复工作流** —— 让 LLM 解读或修补一张图
- **故事模式（实验）** —— 与看板娘 Misca 对话式解题；默认关闭，在「设置 → 通用」中开启
