...
<img src="src-tauri/icons/Square310x310Logo.png" alt="LovelyMiscLab Logo" width="150" height="150" />
...
把杂项操作拖成节点、连成一张数据流图，本地实时执行 —— 现在还能让 **AI 直接驱动**。
...
[主要特性](#-主要特性) · [MCP / AI 调用](#-mcp让-ai-驱动) · [工作区](#️-工作区) · [节点分类](#-节点分类) · [架构](#️-架构) · [开发](#-开发)
...
- **🤖 MCP 服务** —— 内嵌 MCP 服务器，让 Claude Code / Cursor / Codex 等 AI 客户端**直接驱动引擎、读写你正在编辑的画布**（[详见下节](#-mcp让-ai-驱动)）。
- **AI 生成 & 解释流程** —— 自然语言 → 校验过的真实节点图（自动分层布局）；也能让 AI 解释一张图为何这么连、哪里有问题。
- **完整工作区** —— 命令面板、运行历史、撤销/重做、自动保存与最近项目、边上数据预览……（[详见](#️-工作区)）。
...
| ▶️ 执行 | `run_node` · `run_graph`（不带参数即运行当前画布） |
| 🎨 画布 | `get_canvas` · `set_canvas` · `add_node` · `connect` · `set_param` · `remove_node` · `move_node`… |
| 💾 持久化 & AI | `save_workflow` · `load_workflow` · `save_composite_module` · `generate_workflow` |
| ⚙️ 其他 | `get_settings`（脱敏）· `detect_tool` · `ping` |
...
- ⌨️ **命令面板**（`Ctrl/Cmd + K`）—— 快速跳转视图、执行动作
- 🕑 **运行历史 & 运行到节点** —— 回看每次执行、只跑到某个节点为止
- ↩️ **撤销 / 重做** —— 画布编辑全程可回退
- 💾 **自动保存 + 最近项目** —— 断电不丢，快速重开
- 🧭 **帮助面板** & **边上数据预览** —— 连线上直接看流过的值
...
| 工程 & 设置 | `save_project` · `load_project` · `get_settings` · `set_settings` · `detect_tool` |
...
<div align="center"><sub>用 🦀 Rust + ⚛️ React 构建 · Made for CTF misc</sub></div>
