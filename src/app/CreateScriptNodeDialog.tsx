...
export function CreateScriptNodeDialog() {
  ...
  // Esc / backdrop on a filled-in form asks first instead of silently wiping it.
  const requestClose = async () => {
    if (saving) return;
    const touched = !!(name.trim() || commandPath.trim() || argsTemplate.trim() || inputs.length || params.length || outputs.length);
    if (touched && !(await confirmDialog({ title: "放弃这个脚本节点？", message: "已填写的内容不会保存。", confirmText: "放弃", danger: true }))) return;
    close();
  };
  ...
  return (
    <Dialog open onClose={requestClose} dismissible={!saving} className="max-h-[92vh] w-[720px]">
      <div className="flex items-center gap-2 border-b border-border p-4">
        ...
        <button onClick={() => void requestClose()} className="text-muted-foreground hover:text-foreground">
          <X className="h-5 w-5" />
        </button>
      </div>
        ...
        <div>
          ...
          <div className="flex gap-2">
            <select value={commandSel} onChange={(e) => setCommandSel(e.target.value)} className={inputEl}>
              ...
              {TOOLS.map((t) => (
                <option key={t.key} value={`$tool:${t.key}`}>设置中的工具：{t.label}</option>
              ))}
            </select>
