...
import type { PortSpec } from "@/lib/types";
...
export function HelpDialog() {
  ...
  // Opening for a specific node (e.g. a node's "帮助" menu item) jumps to its doc.
  useEffect(() => {
    if (!open) return;
    if (initialId) {
      setTab("nodes");
  ...
  return (
    <Dialog open={open} onClose={close} className="h-[78vh] w-[960px]" ariaLabel="帮助与节点文档">
...
function widgetText(widget: { kind: string; options?: string[]; min?: number; max?: number }) {
  ...
  if (widget.kind === "number" || widget.kind === "slider")
    return `${WIDGET_LABEL[widget.kind]} ${widget.min ?? ""}–${widget.max ?? ""}`;
  return WIDGET_LABEL[widget.kind] ?? widget.kind;
}
...
function ShortcutDoc() {
  const rows = [
    ...
    ["Ctrl+Shift+S", "另存为"],
    ["Ctrl+Enter", "运行整图"],
    ["Ctrl+C / Ctrl+V / Ctrl+X", "复制、粘贴、剪切选中节点（选中文字时复制文字）"],
    ["Ctrl+D", "复制选中子图"],
    ["Ctrl+A", "全选节点"],
