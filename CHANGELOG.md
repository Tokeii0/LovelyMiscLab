...
整体体验修复：解决「用起来怪怪的」—— 工作会丢、失败无反馈、画布操作跨视图误触发、运行语义反直觉、撤销历史被污染、切换视图丢状态。

### 变更

- **运行语义重做**：「运行」（`Ctrl / Cmd + Enter`）= 整图跑一次；「实时」为独立开关，改参自动重算，耗时节点及其下游跳过并标记「需手动运行」。「停止」只取消、保留已完成节点的结果；新增「清除结果」。节点结果随完成即到，取消或失败不再丢弃已算出的结果；上游失败时下游标记为「已跳过」。
- **撤销历史**：快照只记结构（不含运行结果与选中）；一次拖动、一段连续输入各为一步；批量删除为一步；撤销回到保存点时项目恢复为「已保存」。
- **画布常驻**：切换到设置、运行记录等视图不再重置缩放/平移；画布快捷键（Delete、`Ctrl+A/C/V/Z` 等）只在画布视图、焦点不在输入框时生效，`Ctrl+C` 可正常复制文本。
- **故事模式改为实验功能**：默认关闭、入口隐藏，在「设置 → 通用」中开启；页面与立绘按需加载（首屏 `index.html` 3.2 MB → 0.9 MB）。
- 主题默认跟随系统，可在「设置 → 通用」中选择跟随系统 / 浅色 / 深色；默认窗口 1440×900；标题栏整条可拖动，次要画布操作收进「更多」。

### 新增

- 未保存提示（新建、打开、最近项目、模板、AI 生成/修复、清空、关窗）；另存为（`Ctrl / Cmd + Shift + S`）；启动时可恢复自动保存的草稿。
- 全局 toast 提示与统一对话框（Esc 关闭、焦点陷阱）；破坏性操作加确认。
- 拖线到空白处弹出节点搜索并自动连线；右键菜单补充运行 / 运行到此 / 禁用 / 帮助 / 粘贴；新节点落点自动避让。
- AI 上下文上限可配置（默认 128k，Agent 用到 80% 时停止）。
- 前端单元测试（vitest，`pnpm test`）；CI 增加 `cargo fmt --check` 与 UTF-8 检查。

### 修复

- 后端错误不再显示为 `[object Object]`；设置保存失败不再显示「已保存」。
- 模板 / AI 生成 / 清空后 `Ctrl+S` 不再覆盖之前打开的文件；AI Agent 在真正开始搭建时才清空画布。
- AI 调用报错保留服务端返回的原因；无返回内容时报错；未配置 API Key 时给出明确提示；Agent 与一次性生成可使用自定义模块。
- 连线时拒绝成环；连到已占用的输入会替换旧连线。
- 解压节点：加密压缩包缺密码时在画布上报错；单条目 64 MB / 总计 256 MB 上限。
- 故事模式：加密压缩包会询问密码而不是丢弃；请求竞态、失败不回滚、步骤不可取消等问题。
- 读取文件的节点、随机与 AI 节点不再命中过期缓存；缓存键包含运行环境。
- 在线更新仅允许下载本仓库 GitHub Releases 的安装包。

### 移除

- 未使用的 SQLite 数据库与 fs / shell / store / notification 插件及其权限；未使用的前端依赖（react-router-dom、@tanstack/react-query）与模板遗留文件。
