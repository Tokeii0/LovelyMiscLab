...
fn server_end_to_end_over_http() {
    ...
    assert!(
        bad.starts_with("HTTP/1.1 401"),
        "wrong token should 401, got: {}",
        bad.lines().next().unwrap_or("")
    );
    ...
    for expected in [
        ...
        assert!(
            ...
            "tools/list missing {expected}: {tools}"
        );
    }
    ...
    assert!(
        list_resp.contains("base64"),
        "list_nodes(base64) empty: {list_resp}"
    );
    ...
    assert!(
        add_resp.contains("ai_text_input_1"),
        "add_node id wrong: {add_resp}"
    );
    let emitted = bridge.emitted.lock().unwrap();
    assert_eq!(
        emitted.len(),
        1,
        "add_node should emit exactly one canvas update"
    );
