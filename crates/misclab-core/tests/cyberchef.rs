...
fn change_case_ops() {
    ...
    assert_eq!(
        t("change_case", "AbC", json!({ "mode": "交换大小写" })),
        "aBc"
    );
    assert_eq!(
        t("change_case", "hello world", json!({ "mode": "词首大写" })),
        "Hello World"
    );
}
...
fn rail_fence_roundtrip() {
    ...
    assert_eq!(
        t("rail_fence_decode", &e, json!({ "rails": 3 })),
        "WEAREDISCOVEREDFLEEATONCE"
    );
}
...
fn defang_refang() {
    let d = t(
        "defang",
        "http://evil.com/x",
        json!({ "operation": "defang" }),
    );
    assert_eq!(d, "hxxp[://]evil[.]com/x");
    assert_eq!(
        t("defang", &d, json!({ "operation": "refang" })),
        "http://evil.com/x"
    );
}
...
fn des_and_3des_roundtrip() {
    ...
    assert_eq!(
        enc("解密", &ct, "0123456789abcdef", "Hex", "UTF8"),
        "hello123"
    );
    // 3DES (24-byte key)
...
fn blake2b_and_whirlpool() {
    let h = |algo: &str, input: &str| {
        text_of(
            &run("hash", "data", input, json!({ "algorithm": algo })),
            "text",
        )
    };
    assert_eq!(
...
fn gzip_roundtrip() {
    ...
    i.insert(
        "data".to_string(),
        PortValue::Text("hello world hello world".to_string()),
    );
    let comp = GraphExecutor::run_node(
        &reg,
        "compress",
        &i,
        &json!({ "format": "Gzip" }),
...
fn json_substitution_braille() {
    ...
    assert_eq!(
        t(
            "substitution",
            "ABCXYZ",
            json!({ "from": "ABCDEFGHIJKLMNOPQRSTUVWXYZ", "to": rev })
        ),
        "ZYXCBA"
    );
...
fn bifid_and_playfair() {
    ...
    assert_eq!(
        t("bifid_decode", &e, json!({ "keyword": "CIPHER" })),
        "ATTACKATDAWN"
    );
    // Playfair MONARCHY: HI -> BF (rectangle rule).
    assert_eq!(
        t("playfair_encode", "HI", json!({ "keyword": "MONARCHY" })),
        "BF"
    );
    let pe = t(
        "playfair_encode",
        "HIDEGOLD",
        json!({ "keyword": "PLAYFAIREXAMPLE" }),
    );
    assert_eq!(
        t(
            "playfair_decode",
...
fn filetype_detect() {
    assert!(text_of(
        &run("detect_file_type", "data", "%PDF-1.7 body", json!({})),
        "type"
    )
    .contains("PDF"));
    assert!(text_of(
        &run("detect_file_type", "data", "RIFF????WEBP data", json!({})),
        "type"
    )
    .contains("WEBP"));
}
...
fn extract_tokens() {
    let ip = run(
        "extract",
        "text",
        "srv 10.0.0.1 x 8.8.8.8 10.0.0.1",
        json!({ "kind": "IPv4" }),
    );
...
fn quoted_printable_roundtrip() {
    assert_eq!(
        text_of(
            &run("quoted_printable_encode", "data", "café", json!({})),
            "text"
        ),
        "caf=C3=A9"
    );
...
fn sm3_vector() {
    ...
    assert_eq!(
        text_of(
            &run("hash", "data", "abc", json!({ "algorithm": "SM3" })),
            "text"
        ),
...
fn adfgvx_roundtrip() {
    let enc = t(
        "adfgvx",
        "ATTACKAT1200AM",
        json!({ "operation": "加密", "keyword": "PRIVACY" }),
    );
    let dec = t(
        "adfgvx",
        &enc,
        json!({ "operation": "解密", "keyword": "PRIVACY" }),
    );
...
fn timestamp_roundtrip() {
    assert_eq!(
        t("from_timestamp", "1516239022", json!({})),
        "2018-01-18 01:30:22 UTC"
    );
    assert_eq!(
        t("to_timestamp", "2018-01-18 01:30:22", json!({})),
        "1516239022"
    );
}
...
fn jwt_decode_payload() {
    ...
    assert!(
        payload.contains("John Doe") && payload.contains("1234567890"),
        "got {payload}"
    );
}
...
fn hash_crack_dictionary() {
    ...
    let target = text_of(
        &run("hash", "data", "s4ltsecret", json!({ "algorithm": "MD5" })),
        "text",
    );
    let salted = crack(
        &target,
        vec!["x", "secret"],
        json!({ "algorithm": "MD5", "salt": "s4lt", "saltMode": "前缀" }),
    );
...
#[test]
fn string_match_ops() {
    let b = |op: &str, text: &str, value: &str, ic: bool| match run(
        "string_match",
        "text",
        text,
        json!({ "operation": op, "value": value, "ignoreCase": ic }),
    )
    .get("result")
    {
