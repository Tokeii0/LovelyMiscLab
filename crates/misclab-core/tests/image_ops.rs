...
fn channel_extract_r_gray() {
    let m = run(
        "channel_extract",
        &[("data", &solid(1, 1, [100, 50, 25, 255]))],
        json!({ "channel": "R", "output": "灰度图" }),
    );
...
fn bit_plane_bit0() {
    let on = run(
        "bit_plane",
        &[("data", &solid(1, 1, [1, 0, 0, 255]))],
        json!({ "channel": "R", "bit": 0 }),
    );
    assert_eq!(decoded(&on).get_pixel(0, 0).0[0], 255);
    let off = run(
        "bit_plane",
        &[("data", &solid(1, 1, [2, 0, 0, 255]))],
        json!({ "channel": "R", "bit": 0 }),
    );
...
fn threshold_128() {
    let hi = run(
        "threshold",
        &[("data", &solid(1, 1, [200, 200, 200, 255]))],
        json!({ "threshold": 128 }),
    );
    assert_eq!(decoded(&hi).get_pixel(0, 0).0[0], 255);
    let lo = run(
        "threshold",
        &[("data", &solid(1, 1, [50, 50, 50, 255]))],
        json!({ "threshold": 128 }),
    );
...
fn colorspace_ycbcr_y() {
    ...
    let m = run(
        "colorspace_extract",
        &[("data", &solid(1, 1, [10, 20, 30, 255]))],
        json!({ "space": "YCbCr", "component": "分量1(H/Y)" }),
    );
...
fn dft_spectrum_dims() {
    let m = run(
        "dft_spectrum",
        &[("data", &solid(8, 8, [100, 100, 100, 255]))],
        json!({}),
    );
...
fn morphology_dilate_grows() {
    ...
    let m = run(
        "morphology",
        &[("data", &img)],
        json!({ "op": "膨胀", "size": 1, "threshold": 128 }),
    );
    let white = decoded(&m).pixels().filter(|p| p.0[0] > 128).count();
    assert!(
        white > 1,
        "dilate should grow the white region, got {white}"
    );
}
...
fn template_match_locates() {
    ...
    let m = run(
        "template_match",
        &[("image", &img), ("template", &tmpl)],
        json!({}),
    );
    match (m.get("x"), m.get("y")) {
        (Some(PortValue::Number(x)), Some(PortValue::Number(y))) => {
            assert_eq!((*x as u32, *y as u32), (3, 1))
        }
...
fn gif_frame_count() {
    let mut buf = Vec::new();
    {
        ...
        enc.encode_frame(image::Frame::new(solid(4, 4, [200, 100, 50, 255])))
            .unwrap();
    }
...
fn png_fix_manual_dims() {
    let m = run_raw(
        "png_fix",
        "data",
        png(&solid(4, 4, [0, 0, 0, 255])),
        json!({ "mode": "手动", "width": 4, "height": 4 }),
    );
    assert_eq!(decoded(&m).dimensions(), (4, 4));
}
...
fn pixel_values_roundtrip() {
    ...
    let back = decoded(&run_text(
        "values_to_image",
        &text,
        json!({ "channels": "RGBA(4)", "width": 2 }),
    ));
...
fn pixel_extract_hex_gray() {
    let m = run(
        "pixel_extract",
        &[("data", &solid(2, 1, [10, 20, 30, 255]))],
        json!({ "channel": "灰度", "base": "十六进制" }),
    );
...
fn image_to_bits_no_newline() {
    ...
    assert_eq!(
        text_of(
            &run("image_to_bits", &[("data", &img)], json!({ "rows": false })),
            "text"
        ),
        "1001"
    );
    assert_eq!(
        text_of(
            &run("image_to_bits", &[("data", &img)], json!({ "rows": true })),
            "text"
        ),
        "10\n01\n"
    );
...
fn qr_encode_version_margin_dims() {
    ...
    let enc = run_text(
        "qr_encode",
        "hi",
        json!({ "version": 3, "scale": 1, "margin": 0 }),
    );
...
fn qr_encode_custom_colors() {
    ...
    let enc = run_text(
        "qr_encode",
        "x",
        json!({ "dark": "#0000ff", "light": "#ff0000", "margin": 0, "scale": 1 }),
    );
