...
fn logic_gates() {
    let g = |a: bool, bb: bool, op: &str| {
        run(
            "logic",
            HashMap::from([("a".into(), b(a)), ("b".into(), b(bb))]),
            json!({ "op": op }),
        )
    };
...
fn switch_case_picks_branch() {
    ...
    assert_eq!(
        text_of(&run("switch_case", inputs, json!({})), "output"),
        "two"
    );

    let oob = HashMap::from([("selector".into(), n(9.0)), ("default".into(), t("def"))]);
    assert_eq!(
        text_of(&run("switch_case", oob, json!({})), "output"),
        "def"
    );
}
...
fn gate_passes_and_blocks() {
    let pass = run(
        "gate",
        HashMap::from([("value".into(), t("secret")), ("condition".into(), b(true))]),
        json!({}),
    );
...
fn range_generates_sequence() {
    let out = run(
        "range",
        HashMap::new(),
        json!({ "start": 0, "end": 5, "step": 1 }),
    );
    assert_eq!(slist_of(&out, "list"), vec!["0", "1", "2", "3", "4"]);
    let out2 = run(
        "range",
        HashMap::new(),
        json!({ "start": 10, "end": 0, "step": -3 }),
    );
...
fn map_transforms_each() {
    let out = run(
        "map",
        HashMap::from([("list".into(), list(&["ab", "cd"]))]),
        json!({ "op": "大写" }),
    );
...
fn join_reduces_list() {
    let out = run(
        "join_list",
        HashMap::from([("list".into(), list(&["a", "b", "c"]))]),
        json!({ "sep": "逗号" }),
    );
...
fn foreach_pipeline_range_map_filter_join() {
    ...
    let ranged = run(
        "range",
        HashMap::new(),
        json!({ "start": 1, "end": 4, "step": 1 }),
    );
    let seq = ranged.get("list").cloned().unwrap();
    let mapped = run(
        "map",
        HashMap::from([("list".into(), seq)]),
        json!({ "op": "大写" }),
    );
